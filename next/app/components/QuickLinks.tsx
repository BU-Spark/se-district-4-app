...
//           <button className="bg-[#1A2C56] text-white py-3 px-4 flex justify-between items-center rounded-md w-full text-sm font-light lora-light">
    ...
    <section className="mt-6 grid grid-cols-2 md:grid-cols-3 gap-6 md:gap-12 px-6 md:px-32 py-6">
      {links.map(({ text, icon, href, link }) => (
        link ? (
          ...
          <Link key={text} href={href} className="w-full">
            <button className="bg-[#1A2C56] text-white py-3 px-4 flex justify-between items-center rounded-md w-full text-sm font-light lora-light">
              <span className="ml-2">{text}</span>
              <span className="mr-2">{icon}</span>
            </button>
